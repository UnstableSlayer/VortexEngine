...
namespace Vortex
{
		...
		: Layer("EditorLayer")
	{
		...
		//Doom Wall
		{
			...
			renderer2DComponent.m_Texture = Vortex::Texture2D::Create("Assets/Textures/testTexture0.png", Vortex::TextureFormat::RGB16);
		}
		...
		//Spiderman
		{
			...
			auto& renderer2DComponent = obj.AddComponent<Vortex::SpriteComponent>();
			renderer2DComponent.m_Texture = Vortex::Texture2D::Create("Assets/Textures/testTexture1.png", Vortex::TextureFormat::RGBA16);
		}
		...
		//Skull
		{
			...
			auto& renderer2DComponent = obj.AddComponent<Vortex::SpriteComponent>();
			renderer2DComponent.m_Texture = Vortex::Texture2D::Create("Assets/Textures/testTexture.png", Vortex::TextureFormat::RGBA16);
		}
		...
		//Background Grid
		{
			...
			auto& renderer2DComponent = obj.AddComponent<Vortex::SpriteComponent>();
			renderer2DComponent.m_Texture = Vortex::Texture2D::Create("Assets/Textures/bg0.png", Vortex::TextureFormat::RGBA16);
	...
	void EditorLayer::OnUpdate()
	{
		...
		for (int i = 0; i < m_Scene.size(); i++)
		{
			...
			/*const glm::vec4 cameraRect = m_Camera.GetComponent<CameraComponent>().GetRect();
			const glm::vec3 cameraPos = m_Camera.GetComponent<TransformComponent>().GetPosition();
	...
	{
		m_Camera.GetComponent<CameraComponent>().OnEvent(event);
	}
		...
		if (dockingEnabled)
		{
			...
			//Viewport
			{
				...
				if (viewportFbSize != *(glm::vec2*)&viewportSize)
				{
					...
					m_Camera.GetComponent<CameraComponent>().ResizeFB(viewportFbSize.x, viewportFbSize.y);
				}
				...
				for (size_t i = 0; i < m_Scene.size(); i++)
				{
					...
					if (ImGui::CollapsingHeader(name))
					{
						...
						if (object->HasComponent<TransformComponent>())// && ImGui::CollapsingHeader("Transform"))
						{
							...
							glm::vec3 rotation = glm::vec3(1.f) * transform.GetRotation();
