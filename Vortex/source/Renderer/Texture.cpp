...
namespace Vortex
{
    ...
    Ref<Texture2D> Texture2D::Create(const uint32_t width, const uint32_t height, TextureFormat format, TextureWrap wrap, TextureFilter filter)
    {
        switch (RendererAPI::GetAPI())
        {
        case RendererAPI::API::None: VORTEX_ASSERT(false, "No API Selected!"); return nullptr;
        case RendererAPI::API::OpenGL: return MakeRef<OpenGLTexture2D>(width, height, format, wrap, filter);
        }
